    ...
    try:
        ...
        try:
            ...
            if not available_models:
                return "利用可能なモデルが見つかりませんでした。APIキーまたは権限を確認してください。"

        except Exception as model_error:
            print(f"DEBUG: Model listing error: {str(model_error)}")
            return f"モデル一覧の取得でエラーが発生しました: {str(model_error)}"

        # 最初に見つかったモデルを使用
        if available_models:
            model_name = available_models[0].split('/')[-1]  # パスから名前のみ取得
            print(f"DEBUG: Using model: {model_name}")
            # model = genai.GenerativeModel(model_name)

            # モデルを gemini-2.5-flash に固定する
            model_name = "gemini-2.5-flash"
            model = genai.GenerativeModel(model_name)

        else:
            return "使用可能なモデルが見つかりませんでした。"
        ...
        return f"申し訳ありません。AIとの会話でエラーが発生しました。エラー詳細: {str(e)}"
